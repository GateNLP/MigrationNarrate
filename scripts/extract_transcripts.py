...
def transcribe_to_txt(model, media_path: Path) -> bool:
    ...
    try:
        ...
        print(
            f"[WARN] ffmpeg not found; cannot process {media_path.name}.",
            file=sys.stderr
        )

    except Exception as e:
        print(
            f"[ERROR] {media_path.name}: {e}",
            file=sys.stderr
        )
...
def load_model(model_size: str, prefer_gpu: bool = True):
    ...
    if prefer_gpu:
        try:
            print(
                f"Loading model '{model_size}' on GPU...",
                file=sys.stderr
            )
    ...
    print(
        f"Loading model '{model_size}' on CPU "
        f"(int8, {cpu_threads} threads)...",
        file=sys.stderr
    )
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python extract_transcripts.py "
            "/path/to/media [model_size] [--cpu]",
            file=sys.stderr
        )
    ...
    # Model used in MigrationNarrate
    model_size = "large-v3"
    force_cpu = False
    ...
    if not files:
        print(
            f"No media files found under {root}",
            file=sys.stderr
        )
        sys.exit(0)
    ...
    print(
        ...
        f"→ {todo} need transcription, "
        f"{already} already have .txt.",
        file=sys.stderr
    )

    if todo == 0:
        print(
            "Nothing to do. All files already have .txt.",
            file=sys.stderr
        )
        sys.exit(0)
